...
struct _FlutterInappwebviewLinuxPlugin {
  ...
  std::unique_ptr<flutter_inappwebview_plugin::HeadlessInAppWebViewManager>
...
void flutter_inappwebview_linux_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  // WPE 时代的 VM 软渲染预检（LIBGL_ALWAYS_SOFTWARE）已随 WPE 后端移除：
  // GPU 直通能力由 WebKitGpuCapture::IsSupported 显式探测，不满足时显式报错。
  ...
  plugin->plugin_instance =
      std::make_unique<flutter_inappwebview_plugin::PluginInstance>(registrar);
  ...
  plugin->cookie_manager =
      std::make_unique<flutter_inappwebview_plugin::CookieManager>(pluginInstance);
  ...
  plugin->proxy_manager =
      std::make_unique<flutter_inappwebview_plugin::ProxyManager>(pluginInstance);
  pluginInstance->proxyManager = plugin->proxy_manager.get();

  plugin->web_storage_manager =
      std::make_unique<flutter_inappwebview_plugin::WebStorageManager>(pluginInstance);
  pluginInstance->webStorageManager = plugin->web_storage_manager.get();

  plugin->webview_environment =
      std::make_unique<flutter_inappwebview_plugin::WebViewEnvironment>(pluginInstance);
